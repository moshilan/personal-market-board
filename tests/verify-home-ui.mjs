...
try {
  for (const width of [360, 393]) {
    ...
    assert.equal(await page.locator('.market-closed-page-note').count(), 0)
    assert.equal(await page.locator('#app').innerText().then((text) => /休市|最近交易日/.test(text)), false)
    ...
    assert.deepEqual(await pairedSelection.locator('.trend-selection-values p').allTextContents(), ['国际黄金折算　950.00 元/克', '国内黄金　980.00 元/克'])
    ...
    await pairedTrend.tap({ position: { x: 0.9 * 320, y: 85 } })
    await pairedSelection.waitFor({ state: 'visible' })
    await pairedSelection.focus()
    await pairedSelection.press('Space')
    await pairedSelection.waitFor({ state: 'hidden' })
    await pairedTrend.tap({ position: { x: 0.9 * 320, y: 85 } })
    await pairedSelection.waitFor({ state: 'visible' })
    ...
    assert.deepEqual(fuelSelectedRows.map((row) => row.split('　')[0]), ['92号汽油', '95号汽油', '0号柴油'])
