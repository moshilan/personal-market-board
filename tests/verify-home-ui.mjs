...
try {
  for (const width of [360, 393]) {
    ...
    await assert.doesNotReject(() => page.getByText(/^历史数据积累中，国际金价折算\d+条，国内金价\d+条$/, { exact: true }).waitFor())
    await assert.doesNotReject(() => page.getByText(/^(暂无历史记录，等待下一次采集|当前仅有1条历史记录，数据积累中)$/, { exact: true }).waitFor())
    ...
    assert.equal(singlePointGoldDots.filter((color) => color === '#205c50').length, 1)
    ...
    await assert.doesNotReject(() => page.getByRole('heading', { name: '品牌金价趋势', exact: true }).waitFor())
    await assert.doesNotReject(() => page.getByRole('heading', { name: '四品牌金价', exact: true }).waitFor())
